...
c & d-->z
b-->z["CareOS-ML"]--> s("selected patients for CareOS Management")
```

## Our Approach
**CareOS-ML** combines foundation model knowledge with client-specific calibration, producing predictions tuned to the idiosyncrasies of each clinic or health system. Our enables partnerships with organizations that would otherwise lack sufficient data for standalone model development. A regional clinic with 5,000 eligible patients benefits from patterns learned across 200,000+ patients while still receiving predictions calibrated to their specific context.


## How does it work ?

**1. Foundation models** are trained on Saffron's Golden Data Set (≥200,000 women with linked claims and EHR data), learning generalizable patterns for perimenopause and menopause detection across diverse populations and care settings. Saffron incorporates each partner's local data (<20,000 women is sufficient) to adjust for site-specific documentation practices, coding conventions, patient demographics, and care patterns.
...
**3. Quantifying uncertainty.** Most models produce point estimates: "78% probability of undiagnosed perimenopause." But clinical utility depends on knowing whether that estimate is reliable. Two patients scored at 78% may have very different underlying certainty—one with rich documentation and clear symptom patterns, another with sparse records and ambiguous presentation. Our model distinguishes these cases, providing confidence bounds that inform clinical prioritization.
...
| **Our approach** | Learns from labeled and unlabeled cases; calibrated probabilities with confidence bounds; interpretable feature contributions; adapts to small partner populations via transfer learning |
...
The system produces a prioritized worklist of patients with likely undiagnosed perimenopause or menopause. Each patient record includes:
...
Providers review a curated set of high-yield cases rather than sifting through marginal candidates. The model updates daily, ensuring newly presenting patients are identified promptly.
...
This approach addresses a genuine gap in women's health by:

- Learning from the inherent structure of incomplete documentation rather than requiring exhaustive labeling
- Providing calibrated uncertainty that maps directly to clinical decision thresholds
- Delivering precision-optimized worklists that preserve provider trust and concentrate effort on actionable cases
- Enabling partnerships with organizations of any size through foundation models that adapt to local data characteristics

The infrastructure scales to populations of one million patients with daily refresh cycles, while the transfer learning architecture allows partners with as few as 5,000–20,000 patients to benefit from models trained on 200,000+.
